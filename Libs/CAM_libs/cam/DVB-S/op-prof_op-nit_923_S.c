...
int main(void)
{
   ...
   printf("[BOLD]OP op-prof_op-nit_923_S Start \n");   
   ...
   /* set DVB-S Big Nit Buffer from CICAM MFS  */   
   stNitBuffer_now.pu8NitBuffer = NULL;
   strncpy(stNitBuffer_now.au8NitLabel, "op_nit_9022_S", 16); /* set nit_label */
   stNitBuffer_now.au8NitLabel[16] = '\0';
   stNitBuffer_now.u16NitSize = 0;   
   stNitBuffer_now.eDeliverySystemDesc = DELIVERY_S;
   cit_op_SetBigCicamNit(&stNitBuffer_now);
   
   /* op_nit_2000_S */
   u8NitVersion = 1; /* +1 already done above */
   memcpy(stNitBuffer_after.au8NitBuffer, op_nit_2000_S, sizeof(op_nit_2000_S));
   stNitBuffer_after.au8NitBuffer[5] |= ((u8NitVersion << 1) & 0x3E); /* update correct Nit Version */
   strncpy(stNitBuffer_after.au8NitLabel, "op_nit_2000_S", 16); /* set nit_label */
   stNitBuffer_after.au8NitLabel[16] = '\0';
   stNitBuffer_after.u16NitSize = sizeof(op_nit_2000_S);   
