...
int main(void)
{
   ...
   printf("[BOLD]OP op-prof_op-nit_923_C Start \n");   
   ...
   /* set DVB-C Big Nit Buffer from CICAM MFS  */   
   stNitBuffer_now.pu8NitBuffer = NULL;
   strncpy(stNitBuffer_now.au8NitLabel, "op_nit_9022_C", 16); /* set nit_label */
   stNitBuffer_now.au8NitLabel[16] = '\0';
   stNitBuffer_now.u16NitSize = 0;   
   stNitBuffer_now.eDeliverySystemDesc = DELIVERY_C;
   cit_op_SetBigCicamNit(&stNitBuffer_now);
   
   /* op_nit_2000_C */
   u8NitVersion = 1; /* +1 already done above */
   memcpy(stNitBuffer_after.au8NitBuffer, op_nit_2000_C, sizeof(op_nit_2000_C));
   stNitBuffer_after.au8NitBuffer[5] |= ((u8NitVersion << 1) & 0x3E); /* update correct Nit Version */
   strncpy(stNitBuffer_after.au8NitLabel, "op_nit_2000_C", 16); /* set nit_label */
   stNitBuffer_after.au8NitLabel[16] = '\0';
   stNitBuffer_after.u16NitSize = sizeof(op_nit_2000_C);   
