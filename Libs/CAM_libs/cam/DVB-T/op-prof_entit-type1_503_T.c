...
/* op_nit_2003_T */
uint8 op_nit_2003_T[78] = {
0x40,0xF0,0x4F,0x00,0x86,0xC1,0x00,0x00,0xF0,0x06,0x40,0x04,0x44,0x54,0x56,0x4C,0xF0,0x3C,0x00,0x01,
0x00,0x85,0xF0,0x36,0x5A,0x0B,0x02,0xD3,0x44,0x40,0x1F,0x80,0x0A,0xFF,0xFF,0xFF,0xFF,0x5F,0x04,0x00,
...
int main(void)
{
   ...
   printf("[BOLD]OP op-prof_entit-type1_503_T Start \n"); 
   ...
   memcpy(stNitBuffer.au8NitBuffer, op_nit_2002_T, sizeof(op_nit_2002_T));
   stNitBuffer.au8NitBuffer[5] |= ((u8NitVersion << 1) & 0x3E); /* update correct Nit Version */
   strncpy(stNitBuffer.au8NitLabel, "op_nit_2002_T", 16); /* set nit_label */
   stNitBuffer.au8NitLabel[16] = '\0';
   stNitBuffer.u16NitSize = sizeof(op_nit_2002_T);   
   /* set nit_buffer_now */
   cit_op_SetNitBufferNow(&stNitBuffer);

   /* op_nit_2003_T */
   u8NitVersion = stStatusBody_after.u8NitVersion; /* +1 already done above */
   memcpy(stNitBuffer.au8NitBuffer, op_nit_2003_T, sizeof(op_nit_2003_T));
   stNitBuffer.au8NitBuffer[5] |= ((u8NitVersion << 1) & 0x3E); /* update correct Nit Version */
   strncpy(stNitBuffer.au8NitLabel, "op_nit_2003_T", 16); /* set nit_label */
   stNitBuffer.au8NitLabel[16] = '\0';
   stNitBuffer.u16NitSize = sizeof(op_nit_2003_T);   
