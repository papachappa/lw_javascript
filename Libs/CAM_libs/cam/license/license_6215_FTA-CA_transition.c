...
/* ---------------------------------------------------------------------------- */ 
/* HTR 6.2.1.5: FTA Transitions While Recording,
 *              Valid Recording
 * Script name: license_6215.c
 * Preconditions:
 * 
 * Description:
 * - This script supports HTR 6.2.1.5.
 */
#include <picoc_cit.h>
...
int main()
{
   ...
   uint8              u8UriRL = 0;
   uint8              u8EventId = 0;
   uint8              *pu8LicenseLabel = "LicenseB1";
   cit_license_Object stLicenseEvt;

   printf("HTR 6.2.1.5.: FTA Transitions, Valid Recording \n");
   ...
   /* Enable CAPMT logging */
   cit_sys_EnableCapmtDump(1);
   printf("CAPMT logs are enabled \n");
